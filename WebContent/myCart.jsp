  	 ...
  	 <table border="1" bordercolor="blue" cellspacing="0" align="center" width="80%">
  		<tbody id="tbody">
			...
			<c:forEach items="${sessionScope.cart }" var="c">
	  			<tr align="center">
	  			  <td><input type="checkbox" name="productIds" value="${c.value.product.id }"/></td>
	  	          <td>${c.value.product.id }</td>
	  	          <td>${c.value.product.productName }</td>
			       <td><img src="${pageContext.request.contextPath}${c.value.product.picpath }"/></td>
	  	           <td>${c.value.product.price }</td><td>
	  	      	    <input type="button" value="-" onclick="sub(this);"/><input type="text" value="${c.value.amount }" size="1" maxlength="1" name="1" style="color:black;"/><input type="button" value="+" onclick="add(this);"/>
	  	          </td><td>${c.value.totalPrice }</td>
